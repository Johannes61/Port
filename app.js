...
function flipToggle(toGlass){
  ...
  // Toggle into/out of the "glass" state
  nav.classList.toggle('nav-glass', toGlass);

  // LAST: final box
  const last = nav.getBoundingClientRect();

  // Invert from centers to avoid lateral drift
  const dx = (first.left + first.width/2) - (last.left + last.width/2);
  const dy = (first.top + first.height/2) - (last.top + last.height/2);
  const sx = first.width / last.width;
  const sy = first.height / last.height;

  // Animate container forming in place
  nav.animate([
    {
      transformOrigin: 'center top',
      transform: `translate(${dx}px, ${dy}px) scale(${sx}, ${sy})`,
      borderRadius: toGlass ? '0px' : '16px',
      backdropFilter: 'blur(0px)'
    },
    {
      transformOrigin: 'center top',
      transform: 'translate(0,0) scale(1,1)',
      borderRadius: toGlass ? '16px' : '0px',
      backdropFilter: toGlass ? 'blur(10px)' : 'blur(0px)'
    }
  ], { duration: 380, easing: 'cubic-bezier(0.22,0.61,0.36,1)' });

  // Tuck nav contents slightly inward
...
// Quick Actions state (consistent purple)
...
// Skills marquees (seamless; middle lane reversed)
const Adobe = [
...
const Code = [
  ...
  ['py','Py','Python'],['ml','ML','Machine Learning'],['react','Re','React'],
  ['next','Nx','Next.js'],['node','Nd','Node.js'],['db','DB','PostgreSQL']
];
const General = [
...
// Footer year + demo visitors (shared across tabs)
